...
def tune_amplifiers_feedback(program, start_input, start_phase, end_phase):
    ...
    Runs the intcode program through each amplifier (5 total) in feedback mode , with every 
    permutation of phases, in order to find the permutation that gives the 
    highest output. The output of each amplifier is fed into the next one, along
    with its phase.
    
    In feedback mode the output of the last amplifier is fed back into the first amplifier,
    unless the last amplifier returns out_code 99 indicating that the program has completed.
    
    Whenever an amplifier outputs a signal (opcode 4 in intcode), the program state is halted at
    that point, and then resumed from that point on the next loop.
